...
{
	void d_mmid(double y[], double dydx[], int nvar, double xs, double htot,
		int nstep, double yout[], void (*derivs)(double, double[], double[]));
	void d_pzextr(int iest, double xest, double yest[], double yz[], double dy[],
		int nv);
	int i,iq,k,kk,km;
	...
	for (;;) {
		for (k=1;k<=kmax;k++) {
			...
			d_mmid(ysav,dydx,nv,*xx,h,nseq[k],yseq,derivs);
			xest=SQR(h/nseq[k]);
			d_pzextr(k,xest,yseq,y,yerr,nv);
			if (k != 1) {
				errmax=TINY;
